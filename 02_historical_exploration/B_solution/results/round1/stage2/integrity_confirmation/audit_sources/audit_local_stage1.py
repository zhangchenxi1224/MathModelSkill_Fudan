...
def audit_arm(case, variant, directory, runner_hash, *, arm_definition=None, manifest=None):
    issues = []
    ...
    expected = {"variant": variant, "policy_core_sha256": CORE_SHA, "runner_sha256": runner_hash,
                "error_field_sha256": digest(case["error_field"]), "config": arm_definition["solver_config"] if arm_definition else expected_config(case["problem"], variant),
                "environment": "self_built_fixed_scenario", "source_total": len(case["sources"])}
    if arm_definition is not None:
        expected.pop("runner_sha256")
        expected.update(arm_sha256=digest(arm_definition), solver_class=arm_definition["solver_class"],
                        nosignal_config=arm_definition.get("nosignal_config"), stage=manifest["stage"],
                        manifest_sha256=manifest["manifest_sha256"], dependency_sha256=manifest["dependency_sha256"])
...
def check_development_reports(directory, audited, *, baseline=BASELINE, partition="development"):
    ...
    for row in pairs:
        ...
        expected = {"partition": partition, "baseline_variant": baseline,
