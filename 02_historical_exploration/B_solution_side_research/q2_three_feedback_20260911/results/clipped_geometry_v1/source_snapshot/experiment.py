...
def main():
    ...
    p.add_argument('--context-ids',nargs='*',help='explicit archived public-first context IDs for geometry stress experiments')
    args=p.parse_args()
    ...
    if args.context_ids:
        selected=set(args.context_ids)
        cases=[{'id':c['context_id'],'station':c['first']['position'],'report_deg':c['first']['report_deg'],
                'official_case_id':c['case_id']} for c in read(ROOT/'inputs/official_first_contexts.json')['contexts']
               if c['context_id'] in selected]
        if {c['id'] for c in cases}!=selected: p.error('unknown context ID')
