"""Run the frozen selected P3/P4 policies on five official practice cases each."""
...
def run(out,robot_id):
    ...
    selection=e.read(ROOT/'campaign/deployment_selection.json')
    ...
    for problem in (3,4):
        ...
        for index in range(1,6):
            cases.append(dict(case_id=f'latest-p{problem}-{index:03d}',problem=problem,
                 protocol='selected',arm='selected',split='descriptive',stage='official_practice_5',
                 pilot=False,sequence=len(cases),environment='official_practice',
                 variant=name,policy_spec=selection['validation_specs'][name]))
    plan=dict(practice_only=True,formal_authorized=False,planned_cases=10,
              per_problem=5,selection_sha256=selection['selection_sha256'],cases=cases)
    if (out/'plan.json').exists():
        ...
        (out/'selected_input.json').write_bytes((ROOT/'campaign/deployment_selection.json').read_bytes())
    ...
    try:
        ...
        collector.status(out,'stopped_normally',planned_cases=10)
...
if __name__=='__main__':
    ...
    parser.add_argument('--out',type=Path,default=ROOT/'runs/official_practice_5_each')
