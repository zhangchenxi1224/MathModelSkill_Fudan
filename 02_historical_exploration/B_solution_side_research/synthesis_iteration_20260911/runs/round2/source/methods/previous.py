    ...
    def _fallback(self, knowledge):
        ...
        if self.cover_mode == 'original':
            ...
            try:
                ...
                # Geometry construction may decline a degenerate hull. Retain
                # the original deterministic finite cover in that case.
                points = optical_fallback_points(knowledge.first_direction)
                if distance(self.client.position, points[-1]) < distance(self.client.position, points[0]):
                    points.reverse()
                certificate = {'kind':'original_strip_on_construction_error',
                               'point_count':len(points), 'construction_error':str(exc)}
        if self.spec.get('cover_order'):
            from .cover_order import reorder_cover
            points, ordering = reorder_cover(points, knowledge.hull, self.client.position,
                                             self.spec['cover_order'])
            # This route can have a different full traversal cost. Keep the
            # geometric cover proof but never reuse its previous route bound.
            if 'worst_case_cost_s' in certificate:
                certificate['before_reorder_worst_case_cost_s'] = certificate['worst_case_cost_s']
            certificate['worst_case_cost_s'] = ordering['full_traversal_cost_s']
