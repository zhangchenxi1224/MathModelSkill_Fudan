...
def files():
    ...
    for folder in ('methods','vendor/bsolver','vendor/diverse','vendor/dynamic_joint','vendor/dynamic_joint/_core'):
...
def prepare_campaign(args):
    ...
    policy={'baseline':'Q3 previous p3_hull_r1150; Q4 prior confirmed combined_cover (25 stations, refined local cover, L1).',
            'objective':'Complete clearance first; then mean total virtual seconds on 50/50 H1/H2, per problem.',
            ...
            'validation_adoption':'All selected confirmation and stress runs complete; both H mean differences <=0; problem-level paired 97.5% CI upper <0. Otherwise keep champion; do not retune on confirmation.'}
...
def make_stage(args):
    ...
    if args.partition!='development':
        ...
        if selection.get('protocol_sha256')!=digest(protocol):
            raise ValueError('campaign protocol changed after selection')
...
def validate_specs(specs):
    champion=specs.get('champion',{})
    if ({k:v for k,v in champion.items() if k!='problems'} != {'implementation':'champion','limit':1}
            or sorted(champion.get('problems',[3,4])) != [3,4]):
        raise ValueError('exact prior-winner control required for both problems')
...
def freeze(args):
    ...
    for problem in (3,4):
        ...
        if choice!='champion':
            spec=dict(plan['specs'][choice])
            spec['problems']=sorted(set(validation.get(choice,{}).get('problems',[]))|{problem})
