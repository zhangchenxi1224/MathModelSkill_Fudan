...
SCHEMA_VERSION = 1
MECHANISM_CHECK_VERSION = "joint-visibility-case-boundaries-v2"
...
def _compare_scalar(official, local, *, metric, repeats=300, seed=0, absolute_tolerance=.1):
    ...
    result["bootstrap_difference_ci95"] = [lo, hi]
    bounded_rate = any(token in metric for token in ("fraction", "probability", "visibility", "ratio")) and all(
        0 <= x <= 1 for x in official + local)
    if bounded_rate:
        # Entire cases are the independent units. For a case-level Y in
        # [0,1], E[Y] <= P(Y>0) and E[Y] >= P(Y=1). At the all-zero/all-one
        # boundary, exact binomial event bounds therefore also bound E[Y].
        # Bonferroni tail allocation is .0125 per population/tail; no action
        # or within-case source is treated as an independent Bernoulli trial.
        def extreme_interval(values):
            if all(v == 0 for v in values):
                return [0., 1 - .0125 ** (1 / len(values))]
            if all(v == 1 for v in values):
                return [.0125 ** (1 / len(values)), 1.]
            return None
        ai, bi = extreme_interval(official), extreme_interval(local)
        if ai is not None or bi is not None:
            result["degenerate_case_rate_boundary"] = True
            if ai is None:
                aboot = [statistics.mean(rng.choices(official, k=len(official))) for _ in range(repeats)]
                ai = [_quantile(aboot, .0125), _quantile(aboot, .9875)]
            if bi is None:
                bboot = [statistics.mean(rng.choices(local, k=len(local))) for _ in range(repeats)]
                bi = [_quantile(bboot, .0125), _quantile(bboot, .9875)]
            lo, hi = min(lo, bi[0] - ai[1]), max(hi, bi[1] - ai[0])
            result["boundary_case_mean_intervals"] = {"official": ai, "local": bi}
            result["boundary_interval_method"] = "exact whole-case extreme-event bounds; nonboundary side uses case bootstrap; widened beyond plug-in difference interval"
...
def _joint_two_sample(official, local, *, repeats=300, seed=0):
    ...
    if result["observed_tv"] > .25 and result["permutation_p"] < .025:
        ...
        if min(len(official), len(local)) < 30 and min(len(set(official)), len(set(local))) == 1:
            result["degenerate_joint_support_warning"] = "sparse single observed composition cannot establish joint-distribution equivalence"
        else:
            result["status"] = "matched"
...
def mechanism_checks(cases, measurements, proxies, contractions, matrices, *, joint_visibility=None, repeats=300):
    ...
    for (candidate, problem, protocol), local in sorted(candidates.items(), key=lambda item: str(item[0])):
        ...
        reports.append({"model_candidate": candidate, "problem": problem, "protocol": protocol,
                        ...
                        "composition_confounding": "joint composition has not shown compatibility; any visible/cost discrepancy may be composition-driven and cannot identify a radius/noise factor" if composition["status"] != "matched" else
                                                   "joint composition compatible within the stated exploratory margin; other latent factors remain confounded",
    ...
    return {"schema_version": 1, "analysis_version": MECHANISM_CHECK_VERSION, "candidate_checks": reports,
...
def mechanism_handoff(cases, model_checks):
    ...
    for candidate, checks in sorted(grouped.items()):
        ...
        supported_factor_bias = any(c["status"] == "biased" and c["joint_composition"]["status"] == "matched" for c in checks)
        status = ("biased" if supported_factor_bias else "matched" if checks and
        ...
        rows.append({"id": candidate, "radius_mode": source.get("radius_mode"), "error_mode": source.get("error_mode"),
                     ...
                     "composition_model_checked": source.get("composition_model"),
                     "full_candidate_bias_without_composition_resolution": any(c["status"] == "biased" for c in checks) and not supported_factor_bias,
...
def distinct_mechanism_candidates(assessments):
    """Avoid counting one physical mechanism twice for two count priors.

    Full composition x mechanism assessments remain separate. This view is
    only the mechanism-axis handoff for a later explicitly designed cross.
    """
    groups = defaultdict(list)
    for row in assessments:
        key = (row.get("radius_mode"), row.get("error_mode"), row.get("correlation_length"))
        groups[key].append(row)
    rows = []
    for (radius, error, correlation), evidence in sorted(groups.items(), key=lambda item: str(item[0])):
        status = ("biased" if any(r["status"] == "biased" for r in evidence) else "matched" if
                  all(r["status"] == "matched" for r in evidence) else "unresolved")
        if status == "biased":
            continue
        scale = f"{correlation:g}" if _number(correlation) else "unlogged"
        row = {"id": f"radius_{radius}__noise_{error}__correlation_{scale}", "radius_mode": radius,
               "error_mode": error, "status": status, "screen_flag": any(r["screen_flag"] for r in evidence),
               "evidence_model_candidates": [r["id"] for r in evidence],
               "composition_models_checked": sorted({r["composition_model_checked"] for r in evidence if r.get("composition_model_checked")}),
               "evidence_groups": [{"model_candidate": r["id"], **group} for r in evidence for group in r["evidence_groups"]],
               "retention_reason": "retain one vote per distinct physical mechanism; unresolved is not validation",
               "cross_product_limit": "only listed composition x mechanism combinations have direct protocol evidence; other later combinations remain assumptions"}
        if correlation is not None:
            row["correlation_length"] = correlation
        else:
            row["missing_parameters"] = ["correlation_length"]
            row["parameter_note"] = "downstream default, if used, remains an assumption"
        rows.append(row)
    return rows
...
def calibrate(roots, output, *, compare_roots=(), exploratory=False, salt=SPLIT_SALT,
    ...
    _write_json(output / "mechanism_candidates.json", distinct_mechanism_candidates(handoff))
