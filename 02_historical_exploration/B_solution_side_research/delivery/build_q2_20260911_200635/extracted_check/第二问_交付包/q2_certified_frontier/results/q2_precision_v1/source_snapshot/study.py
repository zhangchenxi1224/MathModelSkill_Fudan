...
def response(source, radius, position, error):
    ...
    return {'measure_result':'direction','svd_deg':round((bearing_deg(position,source)+error)%360,2)%360}
...
def indistinguishable_pair(k, q):
    ...
    states = []
    for radius in [5.01]+list(range(25,1501,25)):
        for theta in (-.9999,-.5,0.,.5,.9999):
            ...
            r = max(1000.,radius)
            if (math.hypot(*g)<=1800. and distance(g,q)>5 and distance(g,q)<=r):
...
def synthetic(output, options,workers=2):
    ...
    for station,true_angle,ranges in templates:
        for first_error in (-1.,0.,1.):
            ...
            for r in ranges:
                g = (station[0]+r*math.cos(math.radians(true_angle)),
                     station[1]+r*math.sin(math.radians(true_angle)))
    ...
    write(output/'synthetic_summary.json',{'distinct_worlds':len(scenarios),'rows':len(records),
          ...
          'fixed_error_rule':'error1 at the first coordinate, error2 at every other coordinate; no resampling on repeated query',
