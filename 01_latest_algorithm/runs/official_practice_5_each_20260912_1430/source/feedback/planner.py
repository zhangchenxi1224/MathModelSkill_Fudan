...
def proposals(k,current,limit=6):
...
def choose(k,current,current_channel,options,prior=None,remaining=1):
    ...
    def search(state,here,channel,cloud,depth):
        ...
        value=expected_cover(points,here,cloud)
        best={'kind':'cover','point':here,'expected_s':value}
        if depth<=0 or nodes>=node_budget or time.monotonic()>=deadline:return best
        for q in proposals(state,here,options.get('candidate_limit',5)):
            if nodes>=node_budget or time.monotonic()>=deadline:break
            nodes+=1
            cost=distance(here,q)/5+5+int(channel!=state.channel)
            outcomes=[]; valid=True
            for kind,angle,mass,conditional in branches(cloud,q,options.get('angle_bin_deg',8.)):
                if time.monotonic()>=deadline:
                    valid=False;break
                if kind=='near':future=5.
                else:
                    try:
                        child=posterior(state,q,kind,angle,options.get('angle_bin_deg',8.),options.get('planning_region_depth',3))
    ...
    return decision,{'evaluated_actions':nodes,'node_budget':node_budget,'horizon':horizon,'particles':len(states),
