"""Summarize preserved official logs and package the ten practice runs."""
...
def finish(out):
    ...
    for section in payload['sections']:
        for row in section['cases']:
            ...
            plans = [r['planning'] for r in decisions if r.get('event') == 'all_feedback_plan']
    ...
    summary = dict(title='最新版算法官方演练：P3、P4各5局', environment='official_practice',
                   selected_policies=read(out / 'selection.json')['policies'],
                   sections=payload['sections'], diagnostics=details,
                   interpretation='本次为10个新官方演练案例，未与旧算法在相同案例配对；用于检验官方可运行性和实际表现，不能单凭本次均值推断相对提速。')
    ...
    headers = ['题目/序号', '方位/近距离/无信号', '规划次数', '规划保护触发', '移动耗时', '检测耗时', '换频耗时', '清除耗时', '末次清除后耗时', '日志']
...
打开 `report/results.html` 查看 P3、P4 各5局的完整表格、诊断统计和日志链接。
