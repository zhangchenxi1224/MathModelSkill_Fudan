...
def finish(out):
    ...
    for section in payload['sections']:
        for row in section['cases']:
            ...
            detail = dict(case_id=row['case_id'], problem=row['problem'], case_code=row['case_code'],
                          ...
                          opportunity_search_measures=result.get('opportunity_search_measures',0),
                          opportunity_search_discoveries=result.get('opportunity_search_discoveries',0),
                          replaced_stations=result.get('replaced_stations',0),
    ...
    headers = ['题目/序号', '方位/近距离/无信号', '规划次数', '规划保护触发', '移动耗时', '检测耗时', '换频耗时', '清除耗时', '末次清除后耗时', '后验重排', '顺路补测', '顺路搜索检测/发现', '暂缓判断次数', '扫掠补测', '日志']
    ...
    for d in details:
        ...
        values += [str(d['count_route_changes']),str(d['station_information_measures']),f"{d['opportunity_search_measures']}/{d['opportunity_search_discoveries']}",str(d['future_service_deferrals']),str(d['resweep_measures'])]
