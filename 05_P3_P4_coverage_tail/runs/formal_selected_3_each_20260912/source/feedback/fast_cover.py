...
def route_length(points, start=(0., 0.)):
    # Inputs here are already finite float points produced/checked by this module.
    # Preserve the original math.dist calls and summation order.
    ...
    for transpose in (False, True):
        for outer_reverse in (False, True):
            for inner_reverse in (False, True):
                ...
                if key_route in evaluated:
                    continue
                evaluated.add(key_route)
