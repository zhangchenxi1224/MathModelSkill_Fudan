"""Run ten official practice cases per problem with the selected feedback policy."""
...
def report(out):
    ...
    table.SCHEMES = {
        3: '覆盖收尾迭代：' + selection()['policies']['3']['candidate'],
        4: '覆盖收尾迭代：' + selection()['policies']['4']['candidate'],
    }
...
def run(out, robot_id=None):
    ...
    chosen = selection()
    cases = [dict(case_id=f'tail-next-p{p}-{i:03d}', problem=p, protocol='selected_feedback',
                  selected_policy=chosen['policies'][str(p)], split='official_descriptive', pilot=False)
             for p in (3, 4) for i in range(1, 11)]
    if not (out / 'plan.json').exists():
        collector.write_json(out / 'plan.json', dict(practice_only=True, planned_cases=20, per_problem=10, cases=cases))
        ...
        for name in ('selected_policy.py', 'selection.json', 'release.json', 'next_policy.py', 'log_policy.py', 'previous_policy.py', 'tail_policy.py', 'continuum_cover.py', 'count_belief.py', 'run.py', 'run_selected_official.py', 'finalize_official.py'):
            shutil.copy2(ROOT / name, source / name)
        (source / 'reports').mkdir()
        shutil.copy2(ROOT / 'reports/geometry_cover_candidates.json', source / 'reports/geometry_cover_candidates.json')
        (source / 'inputs').mkdir()
        shutil.copy2(ROOT / 'inputs/previous_selection.json', source / 'inputs/previous_selection.json')
        shutil.copy2(ROOT / 'inputs/count_prior.json', source / 'inputs/count_prior.json')
    ...
    try:
        ...
        for case in cases:
            ...
            report(out)
        collector.status(out, 'finished', planned_cases=20)
